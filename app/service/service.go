...
import (
	...
	modelsMqtt "github.com/djvabe/broadlinkac2mqtt/app/mqtt/models"
	modelsRepo "github.com/djvabe/broadlinkac2mqtt/app/repository/models"
	"github.com/djvabe/broadlinkac2mqtt/app/service/models"
	modelsWeb "github.com/djvabe/broadlinkac2mqtt/app/webClient/models"
	"github.com/djvabe/broadlinkac2mqtt/pkg/coder"
	"github.com/djvabe/broadlinkac2mqtt/pkg/converter"
...
func (s *service) GetDeviceAmbientTemperature(ctx context.Context, input *models.GetDeviceAmbientTemperatureInput) error {
	...
	if len(response.Payload) > 4 && response.Payload[4] != 0x07 {
		s.logger.WarnContext(ctx, "it is not a result packet, attempting re-authentication",
			slog.String("device", input.Mac),
			slog.Any("input", response.Payload))

		authInput := &models.AuthDeviceInput{
			Mac: input.Mac,
		}

		if err := s.AuthDevice(ctx, authInput); err != nil {
			s.logger.ErrorContext(ctx, "failed to re-authenticate device",
				slog.String("device", input.Mac),
				slog.Any("error", err))
			return models.ErrorInvalidResultPacket
		}

		s.logger.InfoContext(ctx, "successfully re-authenticated device, retrying command",
			slog.String("device", input.Mac))

		return s.GetDeviceAmbientTemperature(ctx, input)
	}
...
func (s *service) GetDeviceStates(ctx context.Context, input *models.GetDeviceStatesInput) error {
	...
	if response.Payload[4] != 0x07 {
		...
		// Összeállítjuk az AuthDeviceInput-ot a MAC címmel
		authInput := &models.AuthDeviceInput{
			Mac: input.Mac,
		}

		// Meghívjuk az AuthDevice-t
		if err := s.AuthDevice(ctx, authInput); err != nil {
			s.logger.ErrorContext(ctx, "failed to re-authenticate device",
				slog.String("device", input.Mac),
				slog.Any("error", err))
		...
		// Újra lekérjük az állapotot az új kulccsal
		return s.GetDeviceStates(ctx, input)
	}
